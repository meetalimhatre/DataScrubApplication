			...
			actions: {
									...
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("EMAIL") === oObject.EMAIL;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].EMAIL;
									} else {
										sCurrentId = aItemsNotInTheList[0].EMAIL;
									}
				...
				iRememberTheListItem: function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("EMAIL"),
			...
			assertions: {
				...
				theListHeaderDisplaysZeroHits: function () {
					return this.waitFor({
						...
						success: function () {
							Opa5.assert.ok(true, "The list header displays 'DS_USERS (0)'");
						},
				...
				theListShouldHaveAllEntries: function () {
					...
					// retrieve all DS_USERS to be able to check for the total amount
					...
					return this.waitFor({
						...
						success: function (oList) {
							...
							this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "title",
									value: "DS_USERS (" + iExpectedLength + ")"
								}),
								success: function () {
